...
const Lightsabers = (props) => {
  ...
  const saberStyle = color => ({
    color, display: 'inline', margin: '0.2em', fontSize: '20px', fontWeight: 'bold',
  });
  ...
  const saber = (wordList, color) => [wordList.map((word, i) => <p key={i} style={saberStyle(color)}>{word}</p>)];
  ...
  return (
    <div style={{
      minHeight: `${lengthOfSaber}em`, maxHeight: '500px', display: 'flex', alignItems: 'center',
    }}
    >
